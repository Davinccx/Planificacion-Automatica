...
def main():
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for x in drone:
            f.write("(localizacion-dron depot "+x+") ")
            
        f.write("\n")
        for x in crate:
            
            f.write("(localizacion-caja depot "+x+ ") ")

        f.write("\n")
        for x in person:
            loc_random = random.choice(location[1:])
            f.write("(localizacion-persona "+loc_random+" "+x+") ")

        f.write("\n")
        for x in crate:
            f.write("(caja-tiene-contenido "+x+" "+random.choice(content_types)+") ")
        f.write("\n")
